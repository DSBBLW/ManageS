...
public class UserStuController {
		...
		@RequestMapping("/updaaa")
		public ModelAndView upds(int id){
			ModelAndView and=new ModelAndView();
			Userstu list=dao.hid(id);
			and.addObject("list", list);
			and.setViewName("teacher-edit-student");
			return and;
		}
		//修改
		...
		//获取数据绑到修改页面
				@RequestMapping("/updaatt")
				public ModelAndView upda(String email){
					ModelAndView and=new ModelAndView();
					Userstu list=dao.hiaa(email);
					and.addObject("list", list);
					and.setViewName("student-edit");
					return and;
				}
				@RequestMapping(value="/updaas",method=RequestMethod.POST)
				public String updas(HttpSession session,@RequestParam("f1")MultipartFile file,HttpServletRequest request,String sname,String bornDate,String photo) throws IllegalStateException, IOException{
					...
					file.transferTo(f1);
					photo = newname+".jpg";
					dao.updas(sname, bornDate, photo);
